...
// Ok: given explicitly
fun gav(b: B): A? = if (b is A && b is C) <!NI;DEBUG_INFO_SMARTCAST!><!DEBUG_INFO_SMARTCAST!>b<!><!> else null

class My(b: B) {
    // Error!
    val <!IMPLICIT_INTERSECTION_TYPE!>x<!> = if (b is A && b is C) b else null
    // Ok: given explicitly
    val y: C? = if (b is A && b is C) <!NI;DEBUG_INFO_SMARTCAST!><!DEBUG_INFO_SMARTCAST!>b<!><!> else null
    // Error!
    fun <!IMPLICIT_INTERSECTION_TYPE!>foo<!>(b: B) = if (b is A && b is C) b else null
}
