...
fun test() {
    val s : Set<Int> = <!NI;TYPE_MISMATCH!><!TYPE_INFERENCE_EXPECTED_TYPE_MISMATCH!>newList()<!><!>
    use(s)
}
...
fun test1(outA: Out<A>, inB: In<B>) {
    ...
    val b: Two<A, C> = <!TYPE_INFERENCE_EXPECTED_TYPE_MISMATCH!>foo(outA, <!NI;TYPE_MISMATCH!>inB<!>)<!>
    use(b)
}
